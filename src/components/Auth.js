...
function Auth({ navigate, authService, db }) {
  ...
  function checkHanlder(e){
    ...
    const checkPromise = new Promise(function(resolve,reject){
      const trueCheck = document.querySelectorAll("input[type='checkbox']:checked:not(#all_check)");
      ...
      <form className="auth-form">
        ...
        <section className="terms">
          <div className="all_check">
            <input type="checkbox" id="all_check" onChange={e => checkHanlder(e)} />
            ...
            {authData.map(function (data, i) {
              return (
                <li key={i}>
                  ...
                  {
                    data.important === true ? <p className="check_text"><span style={{color:"#ff0000d9"}}>*</span>{data.text}</p> : <p className="check_text">{data.text}</p>
                  }
                </li>
              );
            })}
        ...
        {check ? (
          <button className="btn" onClick={e=>joinHelper(e)}>회원가입</button>
        ) : (
          <div className="un_btn" onClick={e=>joinHelper(e)}>회원가입</div>
        )}
      </form>
    </div>
  );
